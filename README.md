...
#### stdlib_base_dists_levy_mode( mu, c )

Evaluates the [mode][mode] for a [Lévy][levy-distribution] distribution with location parameter `mu` and scale parameter `c`.

```c
double out = stdlib_base_dists_levy_mode( 0.0, 1.0 );
// returns ~0.333
```
